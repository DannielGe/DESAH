...
import utils.datasets_coco as datasets_coco
...
parser.add_argument('--bits', default='16,32,64,128', type=str,help='binary code length (default: 128)')
...
parser.add_argument('--CODE_LEN', default=128, type=int, help='binary code length (default: 16)')
...
class Session:
    def __init__(self):
        ...
        self.train_dataset = datasets_coco.MSCOCO(train=True, transform=datasets_coco.coco_train_transform)
                                                  # img_transform=datasets_coco.coco_train_transform)
        self.test_dataset = datasets_coco.MSCOCO(train=False, database=False, transform=datasets_coco.coco_test_transform)
                                                 # img_transform=datasets_coco.coco_test_transform)
        self.database_dataset = datasets_coco.MSCOCO(train=False, database=True, transform=datasets_coco.coco_test_transform)
                                                     # img_transform=datasets_coco.coco_test_transform)
    ...
    def define_model(self, coed_length):
        ...
        txt_feat_len = datasets_coco.txt_feat_len
    ...
    def eval(self, epoch, bit):
        ...
        #
        # logger.info('MAP50 of Image to Text: %.3f, MAP of Text to Image: %.3f' % (MAP50_I2T, MAP50_T2I))
        # logger.info(
        #     'Best MAP50 of Image to Text: %.3f, Best MAP50 of Text to Image: %.3f' % (self.best_it_50, self.best_ti_50))
        # logger.info('--------------------------------------------------------------------')
        ...
        logger.info('MAP5000 of Image to Text: %.3f, MAP5000 of Text to Image: %.3f' % (MAP5000_I2T, MAP5000_T2I))
        logger.info('Best MAP5000 of Image to Text: %.3f, Best MAP5000 of Text to Image: %.3f' % (
        self.best_it_5000, self.best_ti_5000))
        logger.info('--------------------------------------------------------------------')
...
def main():
    ...
    for bit in bits:
        ...
        if args.EVAL == True:
            ...
            logger.info("改进模型 bits = {} ".format(bit))
