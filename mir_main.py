...
parser.add_argument('--bits', default='128', type=str,help='binary code length (default: 128)')
...
parser.add_argument('--NUM-EPOCH', default=40, type=int, help='hyper-parameter: EPOCH (default: 40)')
...
parser.add_argument('--EPOCH-INTERVAL', default=2, type=int, help='INTERVAL (default: 2)')
parser.add_argument('--EVAL-INTERVAL', default=1, type=int, help='evaluation interval (default:)')
...
class Session:
    def __init__(self):
        ...
        self.database_dataset = datasets_mir.MIRFlickr(train=False, database=True, transform=datasets_mir.mir_test_transform)
    ...
    def train(self, epoch, args):
        ...
        logger.info(str(datetime.datetime.now()) + 'Epoch [%d/%d], alpha for ImgNet: %.3f' % (epoch + 1, args.NUM_EPOCH, self.CodeNet_I.alpha))
        ...
        for idx, (img, F_T, labels, _) in enumerate(self.train_loader):
            ...
            if (idx + 1) % (len(self.train_dataset) // args.batch_size / args.EPOCH_INTERVAL) == 0:
                logger.info(str(datetime.datetime.now()) + 'Epoch [%d/%d], Iter [%d/%d] Loss1: %.4f'
    ...
    def eval(self, epoch, bit):
        ...
        MAP50_I2T = calculate_top_map(qu_B=qu_BI, re_B=re_BT, qu_L=qu_L, re_L=re_L, topk=50)
        MAP50_T2I = calculate_top_map(qu_B=qu_BT, re_B=re_BI, qu_L=qu_L, re_L=re_L, topk=50)

        if (self.best_it_50 + self.best_ti_50) < (MAP50_I2T + MAP50_T2I):
            self.best_it_50 = MAP50_I2T
            self.best_ti_50 = MAP50_T2I

        logger.info(str(datetime.datetime.now()) + 'MAP50 of Image to Text: %.3f, MAP of Text to Image: %.3f' % (MAP50_I2T, MAP50_T2I))
        logger.info(str(datetime.datetime.now()) + 'Best MAP50 of Image to Text: %.3f, Best MAP50 of Text to Image: %.3f' % (self.best_it_50, self.best_ti_50))
        logger.info('--------------------------------------------------------------------')

        MAP5000_I2T = calculate_top_map(qu_B=qu_BI, re_B=re_BT, qu_L=qu_L, re_L=re_L, topk=5000)
        MAP5000_T2I = calculate_top_map(qu_B=qu_BT, re_B=re_BI, qu_L=qu_L, re_L=re_L, topk=5000)

        if (self.best_it_5000 + self.best_ti_5000) < (MAP5000_I2T + MAP5000_T2I):
            self.best_it_5000 = MAP5000_I2T
            self.best_ti_5000 = MAP5000_T2I

        logger.info(str(datetime.datetime.now()) + 'MAP5000 of Image to Text: %.3f, MAP5000 of Text to Image: %.3f' % (MAP5000_I2T, MAP5000_T2I))
        logger.info(str(datetime.datetime.now()) + 'Best MAP5000 of Image to Text: %.3f, Best MAP5000 of Text to Image: %.3f' % (self.best_it_5000, self.best_ti_5000))
        logger.info('--------------------------------------------------------------------')
...
def _logging():
    ...
    logfile = os.path.join(logdir, str(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')) + ' log.log')
...
def main():
    ...
    for bit in bits:
        print("code_length = {}".format(bit))
        logdir = './DESAH/mir/'+ str(bit) + '/'
        args.CODE_LEN = bit
        args.bits = str(bit)
        # print(args)-
        mkdir_multi(logdir)
